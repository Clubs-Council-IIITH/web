...
export default async function RecruitmentSchedule() {
    const recruitmentSchedule = await fetch(getStaticFile("json/recruitmentCalendar.json"), {
        next: { revalidate: 120 },
    });
            ...
            <Grid container spacing={2}>
                {months.map((month) => (
                    <Grid item xs={12} md={6} lg={3} xl={3}>
                        <RecruitmentCard clubs={recruitmentScheduleJSON.filter((val) => val.month === month.toLowerCase())} month={month} />
                    </Grid>
                ))}
            </Grid>
        </Box>
