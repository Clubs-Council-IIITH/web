  ...
  const submitHandlers = {
    ...
    edit: async (data, opts) => {
      ...
      if (res.ok) {
        ...
        router.push(`/manage/inventory/${res.data._id}`);
      } else {
        triggerToast({ ...res.error, severity: "error" });
        setLoading(false);
      }
  ...
  async function onSubmit(formData, opts) {
    ...
    try {
      ...
      if (typeof formData.billOfPurchase === "string") {
        ...
        data.billOfPurchase = await uploadFile(
          formData.billOfPurchase[0],
          bill_filename,
          bill_warnSizeMB,
        );
      } else {
        ...
        >
          <Grid container>
            ...
            <Grid container spacing={2}>
              ...
              {/* quantity + brand side by side */}
              <Grid size={{ xs: 12, sm: 6 }}>
                <ItemQuantityInput control={control} />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
    ...
    <Controller
      ...
      render={({ field, fieldState: { error, invalid } }) => (
        <FormControl fullWidth error={invalid}>
          ...
          <Select labelId="clubid" label="Owner Club *" fullWidth {...field}>
            {clubs
              ?.slice()
...
function ItemQuantityInput({ control }) {
  return (
    <Controller
      name="quantity"
      control={control}
