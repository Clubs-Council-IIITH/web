...
import {
  Button, Typography, IconButton, Tooltip, FormControl, InputLabel, Select, MenuItem, Stack,
} from "@mui/material";
...
export default function BulkEdit({mode = "add"}) {
  ...
  const fetchClubs = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        triggerToast({
          title: "Error", messages: ["Unable to fetch clubs", res.error.messages], severity: "error",
        });
        setClubs([]);
      }
    } catch (error) {
      triggerToast({
        title: "Error", messages: ["Unable to fetch clubs", error.message], severity: "error",
      });
      setClubs([]);
    } finally {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        triggerToast({
          title: "Error", messages: res.error.messages, severity: "error",
        });
      }
    } catch (error) {
      triggerToast({
        title: "Error", messages: ["Failed to fetch members.", error.message], severity: "error",
      });
    } finally {
      setLoading(false);
    }
  ...
  async function submit(finalMembers, action, actionName) {
    ...
    for (const member of finalMembers) {
      ...
      if (res.ok) {
        ...
        failureCount++;
        failureMessages.push(`- Failed to ${actionName} ${member.uid}: ${res.error.messages.join(", ")}`);
      }
    }
    ...
    let toastMessages = [];
    if (successCount > 0) {
      toastMessages.push(`${successCount} out of ${finalMembers.length} member(s) ${actionName}ed successfully.`);
    }
    if (failureCount > 0) {
      toastMessages.push(`${failureCount} out of ${finalMembers.length} member(s) failed to be ${actionName}ed.`);
      toastMessages = toastMessages.concat(failureMessages);
    }
    setLoading(false);

    triggerToast({
      title: successCount > 0 ? failureCount > 0 ? "Partial Success" : "Success!" : "Failure",
  ...
  const submitHandlers = {
    add: (formData) => {
      ...
      const finalMembers = newMembers
        ...
        .map((member) => ({
          uid: member.uid, cid: selectedClub, roles: [{
            name: member.role,
      ...
      updatedMembers.forEach((member) => {
        ...
        if (member.role !== latestRole.name || member.isPoc !== fullMember.poc || member.startYear !== latestRole.startYear || (member.endYear === "-" ? null : member.endYear) !== latestRole.endYear) {
          ...
          finalMembers.push({
            uid: member.uid, cid: selectedClub, roles: newRoles, poc: member.isPoc,
          });
        }
      });
  ...
  return (<form onSubmit={handleSubmit(submitHandlers[mode])}>
    <Typography variant="h3" gutterBottom textTransform="capitalize">
      Bulk {mode} Members
    </Typography>

    {mode === "edit" && (<Typography variant="body2" gutterBottom>
    ...
    {mode === "add" && (<Typography variant="body2" gutterBottom>
    ...
    {!disableClubSelector && <FormControl fullWidth sx={{mb: 3, mt: 4}}>
      ...
      <Select
        ...
        sx={{width: "80%"}}
      >
        {clubsLoading ? (<MenuItem disabled>Loading clubs...</MenuItem>) : (clubs
          ?.slice()
          ?.sort((a, b) => a.name.localeCompare(b.name))
          ?.map((club) => (<MenuItem key={club.cid} value={club.cid}>
            {club.name}
          </MenuItem>)))}
      </Select>
    </FormControl>}

    {mode === "add" && (<Typography variant="subtitle2" color="text.secondary" align="left" sx={{mb: 2}}>
      <u>NOTE</u>:<br/>

      - Please ensure that the members being added do not already exist
    ...
    >
      ...
      - Any invalid entries marked in red will be skipped during submission.
      <br/>
      - Edited entries will be sent to the top.
    </Typography>)}
    ...
    <Controller
      ...
      control={control}
      render={({field: {value, onChange}}) => (<MembersTable
        mode={mode}
        rows={value}
        setRows={onChange}
...
function MembersTable({
  mode, rows = [], setRows = console.log, setAllValid, existingMembers = [],
}) {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
  const {triggerToast} = useToast();
  ...
  const onUpdate = async (row) => {
    ...
    if (rawEnd === null || rawEnd === "" || rawEnd === "-") {
      ...
      row.endYear = clamped > new Date().getFullYear() ? "" : Math.max(clamped, row.startYear);
    }
  ...
  const columns = [{
    ...
    filterable: false,
    sortable: true,
    valueGetter: (value, row) => row.role !== row.originalRole || row.startYear !== row.originalStartYear || row.endYear !== row.originalEndYear ? 1 : 0,
  }, {
    field: "uid",
    ...
    editable: addMode,
    renderCell: (p) => {
      return p.value ? (<Typography
        variant="body2"
        color={addMode || p.row.role !== p.row.originalRole || p.row.startYear !== p.row.originalStartYear || p.row.endYear !== p.row.originalEndYear ? "text.primary" : "text.secondary"}
        sx={{
    ...
    field: "role", headerName: "Role", width: 150, flex: isMobile ? null : 4, editable: true, renderCell: (p) => {
      return p.value ? (<Typography
        variant="body2"
        sx={{
          wordBreak: "break-word",
    ...
    renderCell: (p) => (<Typography
      variant="body2"
      color={p.row?.startYear === p.row?.originalStartYear || p.row?.role !== p.row?.originalRole ? "text.secondary" : "text.primary"}
      sx={{
        display: "flex",
        alignItems: "center",
        px: "5px",
    ...
    renderCell: (p) => (<Typography
      variant="body2"
      color={p.row?.endYear === p.row?.originalEndYear ? "text.secondary" : "text.primary"}
      sx={{
        display: "flex",
        alignItems: "center",
        px: "5px",
    ...
    align: "center",
    width: 100,
    renderCell: (p) => (<Tooltip title={p.row.error || ""} disableHoverListener={p.row.isValid}>
      <span>
        <Icon
    ...
    headerName: "",
    width: 50,
    renderCell: (p) => (<IconButton onClick={() => onDelete(p)} size="small">
      <Icon
        color="error.main"
  ...
  return (<>
    {addMode && (<Button size="small" variant="outlined" onClick={onAdd} sx={{mb: 1}}>
    ...
    <DataGrid
      ...
      onProcessRowUpdateError={(error) => triggerToast({
      ...
      initialState={{
        pagination: {
          ...
          sortModel: addMode ? [] : [{field: "isEdited", sort: "desc"}, {field: "uid", sort: "asc"},],
        }, columns: {
